...
export function OntologyCanvas() {
  ...
  const onReconnect = (oldEdge: Edge, connection: Connection) => {
    ...
    if (oldEdge.id.startsWith("subclass-")) {
      // A taxonomy edge always hangs from its child: retargeting moves the
      // parent, re-sourcing moves the whole subClassOf to another child.
      if (connection.source !== oldEdge.source) updateClass(oldEdge.source, { subClassOf: null });
      updateClass(connection.source, { subClassOf: connection.target });
      return;
    }
    updateObjectProperty(oldEdge.id, { domain: connection.source, range: connection.target });
  };
