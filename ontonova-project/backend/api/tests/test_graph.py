...
async def test_pipeline_gives_up_after_max_retries_and_reports_failure():
    with patch("api.core.graph.generate_structured", new=AsyncMock()) as mock_gen:
        # Relational never produces a valid output -> exhausts the retry budget.
        mock_gen.side_effect = [TAXONOMIST_OUT] + [RELATIONAL_BAD, POPULATOR_OUT] * 5
        events = await _collect_events()

    assert events[-1]["stage"] == "done"
    assert events[-1]["status"] == "failed"
    assert "range" in events[-1]["error"]
...
async def test_validator_still_fails_when_structure_itself_is_invalid():
    # A dangling `domain` on a declared property is a semantic gap repair
    # must NOT paper over — only assertions are ever pruned.
    with patch("api.core.graph.generate_structured", new=AsyncMock()) as mock_gen:
    ...
    assert events[-1]["status"] == "failed"
    assert "Class_Ghost" in events[-1]["error"]
